...
스크린샷 순서가 곧 스토어 노출 순서다. 목록에서 가장 먼저 보이는 것이 설치를
좌우하므로, 이 앱의 핵심인 수신 화면을 1번에 뒀다.

## 카피 원칙

**기능을 하나씩 설명하지 않는다.** 사용자는 키패드가 진짜 같아서, 벨소리를 끌 수
있어서 설치하는 게 아니라 가짜 전화가 필요해서 설치한다. 기능마다 한 장씩 붙이면
스토어 카피가 아니라 제품 소개서가 된다.

**과장하지 않는다.** "진짜와 구분되지 않아요" 같은 주장이나 "일어날 이유가
생겼어요" 같은 이야기 만들기 대신, 무엇을 하는 물건인지 담백한 서술문으로 적는다.
세 장이 같은 어조로 이어지게 둔다.
